...
void setup() {
  Serial.begin(115200);
  pinMode(ledSimulation, OUTPUT);
  digitalWrite(ledSimulation, LOW);
  lcd.init();
  lcd.backlight();
  SPI.begin();
  mfrc522.PCD_Init();
...
void serverConnect(String tag){
  ...
  if (httpCode > 0) {
    ...
    String access = resultJson.substring(65,66);
    ...
    if(access == "1"){ digitalWrite(ledSimulation, HIGH); }
